...
export function validateArchiveData(data: unknown): string[] {
  const errors: string[] = []
  if (!isRecord(data)) {
    return [i18n.t("errors.snapshot.missingData")]
  }

  const projectResult = projectSchema.safeParse(data.project)
  if (!projectResult.success) {
    errors.push(fieldError(i18n.t("entities.project"), projectResult.error))
  } else {
    const projectId = projectResult.data.id
    assertSameProject(
      i18n.t("entities.constraint"),
      data.constraints,
      projectId,
      errors,
    )
    assertSameProject(i18n.t("entities.floor"), data.floors, projectId, errors)
    assertSameProject(
      i18n.t("entities.basemap"),
      data.assets,
      projectId,
      errors,
    )
  ...
  const floors = parseList(
    i18n.t("entities.floor"),
    floorSchema,
    data.floors,
    errors,
  )
  const layers = parseList(
    i18n.t("entities.layer"),
    layerSchema,
    data.layers,
    errors,
  )
  const features = parseList(
    i18n.t("entities.feature"),
    featureSchema,
    data.features,
    errors,
  )
  const routeNodes = parseList(
    i18n.t("entities.routeNode"),
    routeNodeSchema,
    data.routeNodes,
    errors,
  )
  const routeEdges = parseList(
    i18n.t("entities.routeEdge"),
    routeEdgeSchema,
    data.routeEdges,
    errors,
  )
  const assets = parseList(
    i18n.t("entities.basemap"),
    archiveAssetSchema,
    data.assets,
    errors,
  ...
  for (const layer of layers) {
    if (!floorIds.has(layer.floorId)) {
      errors.push(
        i18n.t("errors.snapshot.layerFloorMissing", { name: layer.name }),
      )
    }
  }
  for (const feature of features) {
    if (!layerIds.has(feature.layerId)) {
      errors.push(
        i18n.t("errors.snapshot.featureLayerMissing", {
          id: shortId(feature.id),
        }),
      )
    }
  }
  for (const node of routeNodes) {
    if (!routeLayerIds.has(node.layerId)) {
      errors.push(
        i18n.t("errors.snapshot.nodeNotInRouteLayer", {
          id: shortId(node.id),
        }),
      )
    }
  ...
  for (const edge of routeEdges) {
    if (!routeLayerIds.has(edge.layerId)) {
      errors.push(
        i18n.t("errors.snapshot.edgeNotInRouteLayer", {
          id: shortId(edge.id),
        }),
      )
    }
    for (const [label, ref] of [
      [i18n.t("graph.start"), edge.source],
      [i18n.t("graph.end"), edge.target],
    ] as const) {
      if (ref.kind === "node") {
        if (!routeNodeIds.has(ref.nodeId)) {
          errors.push(
            i18n.t("errors.snapshot.edgeNodeMissing", {
              id: shortId(edge.id),
              label,
            }),
          )
        ...
        errors.push(
          i18n.t("errors.snapshot.edgeFeatureMissing", {
            id: shortId(edge.id),
            label,
          }),
        )
  ...
  for (const asset of assets) {
    if (assetDataPath(asset.id, asset.mime) !== asset.dataPath) {
      errors.push(
        i18n.t("errors.snapshot.assetPathMismatch", {
          id: shortId(asset.id),
        }),
      )
    }
...
function assertSameProject(
  ...
  for (const item of items) {
    if (isRecord(item) && item.projectId !== projectId) {
      errors.push(i18n.t("errors.snapshot.wrongProject", { label }))
      return
    }
  }
}
...
function parseList<T>(
  ...
  if (!Array.isArray(value)) {
    errors.push(i18n.t("errors.snapshot.notArray", { label }))
    return []
  }
  const valid: T[] = []
  for (const item of value) {
    ...
    if (result.success) valid.push(result.data)
      ...
      errors.push(
        i18n.t("errors.snapshot.invalidItem", {
          label,
          detail: firstIssue(result.error),
        }),
      )
  }
  return valid
...
function fieldError(label: string, error: unknown): string {
  return i18n.t("errors.snapshot.invalidItem", {
    label,
    detail: firstIssue(error),
  })
}
      ...
      if (isRecord(issue)) {
        const where =
          Array.isArray(issue.path) && issue.path.length > 0
            ...
            : i18n.t("errors.snapshot.value")
        const message = issue.message
        if (typeof message === "string")
          return i18n.t("errors.snapshot.firstIssue", { where, message })
      }
    }
  }
  return i18n.t("errors.snapshot.unparseable")
}
